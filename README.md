...
**a.** [`s3.tf`] (S3 Bucket Configuration) : Refer to the source code provided above
...
**e.** [`variables.tf`] (Variable Declarations) : Refer to the source code provided above
...
Each file now contains sample resource configurations which i have used to create that project. You can modify the values in [`variables.tf`] to fit your project’s requirements.
...
#### **5\. Create the** [`output.tf`] File

This file is used for defining output values to display important resource information after Terraform apply.

- Refer to the source code provided above
---
#### **6\. Generate SSH Keys (**`devops-key` and [`devops-key.pub`])
...
cd playbooks
```
### **Step 4: Create the** `roles` Directory

Create the `roles` directory inside the **playbooks** directory:
...
### **Step 5: Create the** `install_nginx_playbook.yml` File
...
```bash
[servers]
server1 ansible_host=54.242.53.128
server2 ansible_host=13.223.0.156

[servers:vars]
ansible_user=ubuntu
ansible_ssh_private_key_file=/home/anil/mega-project/Multi-env-lac-project/terraform/devops-key
